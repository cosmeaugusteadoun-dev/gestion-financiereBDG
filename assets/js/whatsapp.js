...
// ============================================================
var CATS_SCOLAIRES = ["Scolarité", "Crèche", "Garderie", "Activités parascolaires", "Fêtes scolaires", "Assurance", "APE"];
var CATS_CANTINE    = ["Cantine", "Cantine crèche", "Goûter"];
...
function buildBilanMessage(enfant, postesEnfant, suiviEnfant) {
  ...
  // Bilan scolaire le plus récent, s'il existe
  const dernierSuivi = (suiviEnfant || []).slice().sort((a, b) => (b.trim || "").localeCompare(a.trim || ""))[0];
  if (dernierSuivi) {
    msg += `📘 *BILAN SCOLAIRE — ${dernierSuivi.trim}*\n`;
    msg += dernierSuivi.is_mat
      ? `Appréciation : *${dernierSuivi.appreciation || "—"}*\n`
      : `Moyenne : *${dernierSuivi.moy != null ? dernierSuivi.moy + "/20" : "—"}*\n`;
    msg += `Comportement : ${dernierSuivi.comportement || "—"} · Absences : ${dernierSuivi.absences || 0} · Retards : ${dernierSuivi.retards || 0}\n`;
    if (dernierSuivi.observation) msg += `Observation : ${dernierSuivi.observation}\n`;
    msg += `\n`;
  }
  ...
  msg += `💰 *SITUATION FINANCIÈRE*\n────────────────────\n\n`;

  function bloc(catsList) {
    let out = "";
    catsList.forEach(cat => {
      ...
      const enRetard = lignes.some(p => posteEnRetard(p));
      out += `• *${cat}* : ${fmtFCFA(du)} / Payé ${fmtFCFA(paye)} / Reste *${fmtFCFA(reste)}*${enRetard ? " — ⚠️ ÉCHÉANCE DÉPASSÉE" : ""}\n`;
    });
    return out;
  }

  const blocScol = bloc(CATS_SCOLAIRES);

  // Services ponctuels crèche (halte-garderie, samedi, nuitée) : montant
  // libre sans échéance fixe, affichés avec la Crèche plutôt qu'à part —
  // une ligne par service effectivement utilisé.
  const lignePonctuel = PONCTUEL_CRECHE_CATS
    .map(cat => {
      const total = postesEnfant.filter(p => p.cat === cat).reduce((s, p) => s + (p.paye || 0), 0);
      return total > 0 ? `• *${cat}* : ${fmtFCFA(total)} réglés\n` : "";
    })
    .join("");

  if (blocScol || lignePonctuel) msg += `📚 *FRAIS SCOLAIRES*\n${blocScol}${lignePonctuel}\n`;
  const blocCant = bloc(CATS_CANTINE);
  if (blocCant) msg += `🍽️ *CANTINE & RESTAURATION*\n${blocCant}\n`;

  // Totaux — uniformes et fournitures exclus (montants variables, hors bilan parent)
  const postesTotal = postesEnfant.filter(p => !p.is_remise && p.cat !== "Uniformes" && p.cat !== "Fournitures");
  const duTot = postesTotal.reduce((s, p) => s + (p.du || 0), 0);
  const payeTot = postesTotal.reduce((s, p) => s + (p.paye || 0), 0);
  const remiseTot = tRemise(postesEnfant);
  const resteTot = Math.max(duTot - payeTot - remiseTot, 0);

  msg += `════════════════════\n`;
  msg += `*TOTAL DÛ      : ${fmtFCFA(duTot)}*\n`;
  msg += `*TOTAL PAYÉ    : ${fmtFCFA(payeTot)}*\n`;
  msg += `*RESTE À PAYER : ${fmtFCFA(resteTot)}*\n\n`;
